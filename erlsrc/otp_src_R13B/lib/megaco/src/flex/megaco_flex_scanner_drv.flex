    ...
    if ((yytext[i] != SP)   && 
      ...
      while (i <= yyleng) { 
        ...
        if (yytext[i] == '=') {
          ...
          name[nameLen] = 0;
          mfs_text_ptr += (nameLen + 1); // Make room for the NULL termination
		       ...
		       char **res_buf, int res_buf_len)
{
  ...
  if (!mfs_error) {
    ...
    if (mfs_text_buf  != NULL) FREE(mfs_text_buf);
    if (mfs_term_spec != NULL) FREE(mfs_term_spec);
